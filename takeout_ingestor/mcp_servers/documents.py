...
mcp = FastMCP("Pixeltable-Documents")

# Base directory for all indexes - updated for takeout
DIRECTORY = 'takeout_documents'
...
def setup_document_index(table_name: str) -> str:
    ...
    try:
        ...
        document_index = pxt.create_table(
            full_table_name,
            {'document': pxt.Document},  # Changed from pdf_file to document
            if_exists='ignore'
        )
        ...
        chunks_view = pxt.create_view(
            ...
            iterator=DocumentSplitter.create(
                document=document_index.document,  # Updated field name
...
def insert_document(table_name: str, document_location: str) -> str:
    ...
    try:
        ...
        document_index.insert([{'document': document_location}])  # Updated field name
